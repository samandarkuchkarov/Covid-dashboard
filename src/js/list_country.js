export class List {
    ...
    async list() {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (this.cases == 'case' && this.per_thousand) {
                sum_cases += data[i].cases;
                country.innerHTML = `<i class = "cases">${data[i].casesPerOneMillion/100}</i> <i class = "flag"></i><i class = "country_name">${data[i].country}</i>`
            } else if (this.cases == 'deaths' && this.per_thousand) {
                sum_cases += data[i].deathes;
                country.innerHTML = `<i class = "cases">${data[i].deathesPerMillion/100}</i> <i class = "flag"></i><i class = "country_name">${data[i].country}</i>`
            } else if (this.cases == 'recovered' && this.per_thousand) {
                sum_cases += data[i].recovered;
                country.innerHTML = `<i class = "cases">${data[i].recoveredPerMillion}</i> <i class = "flag"></i><i class = "country_name">${data[i].country}</i>`
            }
    ...
    sort_by_date() {
        ...
        if (this.cases === 'case' && this.per_thousand) {
            this.data.sort((a, b) => {
                return b.todayCases / b.population * (100000) - a.todayCases / a.population * 100000
            })
            document.querySelector('.global-cases-title').textContent = 'Cases of today'
        }
        if (this.cases === 'deaths' && this.per_thousand) {
            this.data.sort((a, b) => {
                return b.todayDeaths / b.population * (10 ** 5) - a.todayDeaths / a.population * (10 ** 5)
            })
            document.querySelector('.global-cases-title').textContent = 'Deathes of today'
        }
        if (this.cases === 'recovered' && this.per_thousand) {
            this.data.sort((a, b) => {
                return b.todayRecovered / b.population * (10 ** 5) - a.todayRecovered / a.population * (10 ** 5)
            })
            document.querySelector('.global-cases-title').textContent = 'Recover of today'
        }
        for (let i = 0; i < this.data.length; i++) {
            ...
            if (this.cases === 'case' && this.per_thousand) {
                ...
                sum_deathes += this.data[i].todayDeaths;
                sum_recovery += this.data[i].todayRecovered;
                country.innerHTML = `<i class = "cases">${(this.data[i].todayCases/this.data[i].population*100000).toFixed(3)}</i> <i class = "flag"></i><i class = "country_name">${this.data[i].country}</i>`;
                document.querySelectorAll('.flag')[i].style.backgroundImage = `url(${this.data[i].countryInfo.flag})`
            }
            if (this.cases === 'deaths' && this.per_thousand === false) {
            ...
            if (this.cases === 'deaths' && this.per_thousand) {
                ...
                sum_deathes += this.data[i].todayDeaths;
                sum_recovery += this.data[i].todayRecovered;
                country.innerHTML = `<i class = "cases">${(this.data[i].todayDeaths/this.data[i].population*100000).toFixed(3)}</i> <i class = "flag"></i><i class = "country_name">${this.data[i].country}</i>`;
                document.querySelectorAll('.flag')[i].style.backgroundImage = `url(${this.data[i].countryInfo.flag})`
            }
            if (this.cases === 'recovered' && this.per_thousand) {
                sum_cases += this.data[i].todayRecovered;
                sum_deathes += this.data[i].todayDeaths;
                sum_recovery += this.data[i].todayRecovered;
                country.innerHTML = `<i class = "cases">${(this.data[i].todayRecovered/this.data[i].population*100000).toFixed(3)}</i> <i class = "flag"></i><i class = "country_name">${this.data[i].country}</i>`;
                document.querySelectorAll('.flag')[i].style.backgroundImage = `url(${this.data[i].countryInfo.flag})`
            }
    ...
    async list_const() {
        ...
        if (this.per_thousand == false) {
            ...
            for (let i = 0; i < this.data.length; i++) {
                ...
                item_recover.className = 'recovery-list-item';
                document.querySelector('.recovery-list').appendChild(item_recover);
                item.innerHTML = `<i class = "cases">${this.data[i].deathsPerOneMillion}</i> <i class = "flag2"></i><i class = "country_name2">${this.data[i].country}</i>`
                item_recover.innerHTML = `<i class = "cases">${this.data_recover[i].recoveredPerOneMillion}</i> <i class = "flag3"></i><i class = "country_name3">${this.data_recover[i].country}</i>`
                document.querySelectorAll('.flag2')[i].style.backgroundImage = `url(${this.data[i].countryInfo.flag})`
                document.querySelectorAll('.flag3')[i].style.backgroundImage = `url(${this.data_recover[i].countryInfo.flag})`
                console.log(this.data_recover[i].deathsPerOneMillion);
            }
            this.per_thousand = false
        }
    ...
    sort_by_date() {
        ...
        if (this.per_thousand === false) {
            ...
            for (let i = 0; i < this.data.length; i++) {
                ...
                console.log(this.data[i].todayDeaths + 'sam')
                ...
                item_recover.innerHTML = `<i class = "cases">${(this.data_recover[i].todayRecovered/this.data_recover[i].population*100000).toFixed(3)}</i> <i class = "flag3"></i><i class = "country_name3">${this.data_recover[i].country}</i>`
                document.querySelectorAll('.flag2')[i].style.backgroundImage = `url(${this.data[i].countryInfo.flag})`
                document.querySelectorAll('.flag3')[i].style.backgroundImage = `url(${this.data_recover[i].countryInfo.flag})`
            }
        }
